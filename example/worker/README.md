...
It also pushes an event whenever its state changes, so nothing has to poll.
...
`r.Fallback` is wired to an older handler that switches on a `"type"` field in
the request, so an interface that does not use named handlers still works.
